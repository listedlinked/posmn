    ...
    model(0)
{
    ...
    ui->CBTAvailableLabel->setTextFormat(Qt::RichText);
    ...
    ui->BittrexCBTLabel->setTextFormat(Qt::RichText);
...
void tradingDialog::UpdaterFunction(){
    //CBTst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::BuyCBT(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::SellCBT(QString OrderType, double Quantity, double Rate){
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    ui->CBTSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(CBTSupply,'i',8) + "</span><b> CBT</b>");
    ...
    ui->CBTDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:12px; color:blue'>" + str.number(CBTDemand,'i',8) + "</span><b> CBT</b>");
    ...
    switch (index){
                case 0:    //buy tab is active
                    ...
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BtcAvailableLabel, *ui->CBTAvailableLabel, Response, Response2);
                    }
                ...
                case 1: //Cross send tab active
                    ...
                    if((Response.size() > 0 && Response != "Error") && (Response2.size() > 0 && Response2 != "Error")){
                        DisplayBalance(*ui->BittrexCBTLabel, *ui->BittrexBTCLabel, Response, Response2);
                    }
                ...
                case 5://show balance tab
                    ...
                    Response = GetBalance("CBT");
                    if(Response.size() > 0 && Response != "Error"){
                        DisplayBalance(*ui->CBTBalanceLabel,*ui->CBTAvailableLabel_2,*ui->CBTPendingLabel, QString::fromUtf8("CBT"),Response);
                    }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputCBT->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
void tradingDialog::CalculateCSReceiveLabel(){
    ...
    double AvailableCBT = BalanceObject["Available"].toDouble();
...
void tradingDialog::on_Sell_Max_Amount_clicked()
{
    ...
    double AvailableCBT = ResultObject["Available"].toDouble();

    ui->UnitsInputCBT->setText(str.number(AvailableCBT,'i',8));
}
...
{
    double Quantity = ui->BittrexCBTLabel->text().toDouble();
    double Received = 0;
    double Qty = 0;
    double Price = 0;
    QString buyorders = GetOrderBook();
...
void tradingDialog::on_Withdraw_Max_Amount_clicked()
{
    ...
    ui->WithdrawUnitsInput->setText(str.number(AvailableCBT,'i',8));
}
...
void tradingDialog::on_BuyCBT_clicked()
{
    double Rate;
    double Quantity;
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  BuyCBT(Order,Quantity,Rate);
...
void tradingDialog::on_SellCBTBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputCBT->text().toDouble();
    ...
    if (reply == QMessageBox::Yes) {
        ...
        QString Response =  SellCBT(Order,Quantity,Rate);
        QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
        QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
    ...
    if(!ctx.isValid())
    {
        ...
        foreach (const QJsonValue & value, BuyArray)
        {
            ...
            if ( ((Quantity / x) - y) > 0 )
            {
                ...
                QString SellResponse = SellCBT(Order,y,x);
                ...
                QString SellResponse = SellCBT(Order,(Quantity / x),x);
                QJsonDocument SelljsonResponse = QJsonDocument::fromJson(SellResponse.toUtf8());          //get json from str.
                QJsonObject SellResponseObject = SelljsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputCBT_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
