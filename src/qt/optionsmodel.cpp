...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeCarebitAmount"))
        settings.setValue("nAnonymizeCarebitAmount", 1000);
    nAnonymizeCarebitAmount = settings.value("nAnonymizeCarebitAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeCarebitAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeCarebitAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeCarebitAmount:
            return QVariant(nAnonymizeCarebitAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeCarebitAmount:
            nAnonymizeCarebitAmount = value.toInt();
            settings.setValue("nAnonymizeCarebitAmount", nAnonymizeCarebitAmount);
            emit AnonymizeCarebitAmountChanged(nAnonymizeCarebitAmount);
            break;
        default:
            break;
        }
