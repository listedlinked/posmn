...
{
    // Convert Carebit:// to Carebit:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Carebit://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Carebit:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Carebit.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Carebit.lnk
...
{
    return GetAutostartDir() / "Carebit.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Carebit.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Carebit\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Carebit app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Carebit app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Carebit-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Carebit-qt [" + tr("command-line options") + "]                     " + "\n";
