...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeCarebitAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeCarebitAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeCarebitAmount = strAnonymizeCarebitAmount.remove(strAnonymizeCarebitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCarebitAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeCarebitAmount*COIN) nMaxToAnonymize = nAnonymizeCarebitAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeCarebitAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeCarebitAmount));
        strAnonymizeCarebitAmount = strAnonymizeCarebitAmount.remove(strAnonymizeCarebitAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeCarebitAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
